...
def generateMarkdown(relPath, xcspecFilename, locFilename, markdownFilename):
    xcodePluginsResourcesPath = appPath + "/Contents/PlugIns/Xcode3Core.ideplugin/Contents/" + relPath

    # This file contains the user readable name and descriptions of the visible options
    Apple_Clang_strings = plistlib.load(open(xcodePluginsResourcesPath + "English.lproj/" + locFilename,"rb"), fmt=plistlib.FMT_BINARY)

    # load the xcspec file (after converting it from the ASCII plist format to XML) and generate categories, just like Xcode
    tempSpecFile = tempfile.gettempdir() + "/" + xcspecFilename.replace(".", "_").replace(" ","_") + ".xml"
    subprocess.run(["plutil", "-convert","xml1","-o",tempSpecFile,xcodePluginsResourcesPath + xcspecFilename])
